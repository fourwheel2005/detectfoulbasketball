...
def draw_foul_alert_panel(frame, active_alerts: dict):
    ...
    for idx, alert in enumerate(alerts):
        text = f"P{alert['player_id']}  {alert['foul_type']}"
        y = panel_y + 70 + idx * 34
        cv2.putText(
            frame,
            text[:48],
...
def main():
    ...
    while cap.isOpened():
        ...
        if frame_count % BALL_STRIDE == 0:
            if use_ball_model and ball_model:
                ...
                if ball_results[0].boxes is not None:
                    for box, cls in zip(ball_results[0].boxes.xyxy.cpu().numpy(),
                                        ball_results[0].boxes.cls.cpu().numpy()):
                        cls_id = int(cls)
                        if cls_id in (BALL_CLASS_ID, SBALL_CLASS_ID):
                            _cached_balls.append(box)
                        elif cls_id == RIM_CLASS_ID:
        ...
        rim_memory_active = False
        if rim_boxes:
            _last_rim_box = max(rim_boxes, key=lambda b: (b[2] - b[0]) * (b[3] - b[1]))
            _rim_lost_frames = 0
        ...
        if rim_for_gt is not None:
            ...
            ref.set_rim_y(None, rim_x_range=None)
        ...
        for (tid, px1, py1, px2, py2) in persons:
            ...
            y_offset = py1 - 28
            for v in violations:
                cv2.putText(frame, f"!!! {v}", (px1, y_offset),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                y_offset -= 26
                any_violation = True   
                frame_foul_names.add(v)
                frame_foul_events.append({
                    "player_id": tid,
                    "foul_type": v,
                    "frame_index": frame_count,
                })
                active_foul_alerts[(tid, v)] = {
                    "player_id": tid,
                    "foul_type": v,
